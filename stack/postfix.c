...
void convert_postfix(char* infix,char* postfix)
{
	...
	while(infix[i]!='\0')
	{
		...
		if(input_prec(ch)>stack_prec(peep(s,top)))
			  ...
			  pop(s,&top);  //for popping '(' parenthesis , PTN - ')' parenthesis never gets into stack.(since it's input precendence is 0 , at that time , popping from stack into postfix happens)
		  i++;
	}
	while(peep(s,top)!='#')	
        postfix[j++]=pop(s,&top);		

...
void display(struct trienode *curr)
{
	...
	for (i = 0; i < 256; i++)
	{
		if (curr->child[i] != NULL)
		{
			word[length++] = i; // copy the character's index
			if (curr->child[i]->endofword == 1)
			{
				printf("\n");
				for (j = 0; j < length; j++)
					printf("%c", word[j]);	// converts the respective index number to ascii character ... it works for every ascii character present in the string.
			}
...
/*
// this works for only alphabetical trie.
int search(char * pattern,struct trienode *root){

	while(*pattern){
		if(root->child[*pattern-'a']==NULL) return 0;

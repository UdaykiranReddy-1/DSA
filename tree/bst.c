	   ...
	   void inorder(tnode_t *root)
	   {
		...
		int search(tree_t *t, int key)
		{
			...
			else
			{
				...
				else if(curr->data==key) return 1;
...
void delete_recursive_t(tree_t * tree,int key){
	if(tree->root == NULL) return;
	deleterec(tree->root,key);
}

tnode_t* deleterec(tnode_t* root,int key){
	if(root==NULL) return;
	if(key < root->data) root->left=deleterec(root->left,key);
		...
		// //case 2 : node with both child

		// tnode_t* succ = minimum(root->right);
		// root->data = succ->data;

		// // delete inorder successor
		// root->right = deleterec(root->right,succ->data);

		// more optimised way than above method.
		else{
			//case 2 : node with both child
			tnode_t * succparent = root;

			// find successor
			tnode_t* succ = root->right;
		...
		root->data = succ->data;
		// since inorder successor will be the leftmost node in the right subtree. it doesn't have any left subtree .... (PTN - point to be noted)
		if(succparent!=root){

...
void delete_alternate_from1(list_t *);
void delete_alternate_from2(list_t *list);
void reverse(list_t *);
...
int main()
{
	...
	while (1)
	{
		...
		printf("9..Delete alternate node from node1\n");
		printf("10..Delete alternate node from node2\n");
		...
		case 8:
			...
			break;
		case 9:
			printf("Deleting alternate nodes from node1..");
			delete_alternate_from1(&l);
			break;
		case 10:
			printf("Deleting alternate nodes from node2..");
			delete_alternate_from2(&l);
			break;
		case 11:
...
void delete_alternate_from1(list_t *list)
{
	if (list->head == NULL)
		return;
...
void delete_alternate_from2(list_t *list)
{
	if (list->head == NULL)
		return;
	node_t *curr, *prev, *temp;
	curr = list->head->link; // points to second node.
	prev = list->head;			// points to first node.
	while (curr != NULL)
	{
		if (curr->link == NULL)
		{
...
double FindandEvalPolyDerivative(struct polynode* head,double X, int n){
	struct polynode* pres = head;
	double sum=0;
	while(pres!=NULL){
		if(pres->degree!=0) sum+= (pres->coef)*(pres->degree)*pow(X,pres->degree-1);
	}
	return sum;
}
